...
std::vector<std::vector<double>> GNM::generateParamSpace(int Nruns,
                                                         std::vector<double> eta_limits,
                                                         std::vector<double> gamma_limits) {
    /**
     * Create a linear parameter space between the eta and gamma bounds to satisfy the desired number of runs
     */

    int sqrtNruns = static_cast<int>(std::sqrt(Nruns));
    std::vector<std::vector<double>> paramSpace;
    std::vector<double> pValues(sqrtNruns);
    std::vector<double> qValues(sqrtNruns);

    double step_size_p = (eta_limits[1] - eta_limits[0]) / (sqrtNruns - 1);
    double step_size_q = (gamma_limits[1] - gamma_limits[0]) / (sqrtNruns - 1);

    // Generate p and q values
    for (int i = 0; i < sqrtNruns; ++i) {
        pValues[i] = eta_limits[0] + i * step_size_p;
        qValues[i] = gamma_limits[0] + i * step_size_q;
    }

    // Create the parameter space by combining p and q values
    for (const auto &p: pValues) {
        for (const auto &q: qValues) {
            paramSpace.push_back({p, q});
        }
    }
    ...
    switch (model) {
        ...
        case 2: // matching
            for (int i = 0; i < n_nodes; ++i) {
                for (int j = i; j < n_nodes; ++j) {
                    ...
                    K_current[i][j] = K_current[j][i] =
                            intersect_connects > 0 ? ((intersect_connects * 2) / union_connects) + epsilon : epsilon;
                }
            }
            break;
        ...
        case 4: // clu-min
            ...
            for (int i = 0; i < n_nodes; ++i) {
                for (int j = i; j < n_nodes; ++j) {
                    K_current[i][j] = K_current[j][i] =
                            ((clu_coeff_current[i] > clu_coeff_current[j]) ? clu_coeff_current[j]
                                                                           : clu_coeff_current[i]) + epsilon;
                }
            }
            break;
        ...
        case 5: // clu-max
            ...
            for (int i = 0; i < n_nodes; ++i) {
                for (int j = i; j < n_nodes; ++j) {
                    K_current[i][j] = K_current[j][i] =
                            ((clu_coeff_current[i] > clu_coeff_current[j]) ? clu_coeff_current[i]
                                                                           : clu_coeff_current[j]) + epsilon;
                }
            }
            break;
        ...
        case 9: // deg-min
            for (int i = 0; i < n_nodes; ++i) {
                for (int j = i; j < n_nodes; ++j) {
                    K_current[i][j] = K_current[j][i] =
                            ((k_current[i] > k_current[j]) ? k_current[j] : k_current[i]) + epsilon;
                }
            }
            break;
        ...
        case 10: // deg-max
            for (int i = 0; i < n_nodes; ++i) {
                for (int j = i; j < n_nodes; ++j) {
                    K_current[i][j] = K_current[j][i] =
                            ((k_current[i] > k_current[j]) ? k_current[i] : k_current[j]) + epsilon;
                }
            }
            break;
